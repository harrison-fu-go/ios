...
extension DatabaseCenter {
    ...
    static func updateAppInfo(_ appInfoModel: DatabaseModel) {
        let sql = self.sharedInstance.generateUpdateSql(tableName: DatabaseConstants.appInfoRecordsTable,
                                         headerSql: "insert or replace into",
                                         taskModel: appInfoModel)
        let result = self.sharedInstance.executeUpdate(sql: sql)
        if !result {
            print("============== updateAppInfo ERROR =============")
        }
    }
    
    static func getLatestAppInfo() -> DatabaseModel? {
        let sql = "SELECT * FROM \(DatabaseConstants.appInfoRecordsTable) order by versionCode desc limit 1;"
        let models = self.sharedInstance.queryDatabaseModel(table: DatabaseConstants.appInfoRecordsTable, sql: sql)
        if models.isEmpty {
            return nil
        }
        return models[0]
