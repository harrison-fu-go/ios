...
class BLECenter: NSObject, CBCentralManagerDelegate, CBPeripheralDelegate, BLETasksCenterProtocol {
    ...
    func scan(_ isEnable: Bool,
        ...
        DispatchQueue.global().async {
            ...
            if isEnable {
                ...
                var scanServiceUUIDs:[CBUUID]?
                if let scanService = scanService {
                    scanServiceUUIDs = self.changeToUUIDD(servers:[scanService])
                }
                self.bleCenter.scanForPeripherals(withServices:scanServiceUUIDs, options: nil)
    ...
    func searchDeviceWithServiceUUIDs(uuids: [String],
                                      enable: Bool = true,
                                      timeout: Int = 20,
                                      scanService:String? = nil,
                                      deviceName:String?  = nil,
                                      callback:BLECALLBACK? = nil) {
        self.scanDeviceName = deviceName
        self.searchDeviceCallback = {device in
            if let deviceName = self.scanDeviceName {
                if device.blePeripheral?.name?.lowercased() != deviceName.lowercased() {
                    return
                }
            }
        ...
        self.scan(enable, nil, timeout, uuids, scanService, nil)
    }
    ...
    /**
     connect device.
     */
    func iConnectWithServiceUUID(uuids: [String],
                                 timeout:Int = 20,
                                 scanService:String? = nil,
                                 deviceName:String? = nil,
                                 callback: BLECALLBACK? = nil) {
        ...
        self.searchDeviceWithServiceUUIDs(uuids: uuids, enable: true, timeout: timeout, scanService: scanService, deviceName: deviceName) { message in
            self.searchDeviceCallback = nil
        ...
        if deviceLsit.count > 0 {
            ...
            if central.state == .poweredOn && !self.isManualDisconnected {
                ...
                DispatchQueue.global().async {
                    ...
                    self.iConnectDevice(device: aDev)
    ...
    //notify or indicate
    func notify(characteristic:CBCharacteristic, value: Bool) {
    ...
    func peripheralIsReady(toSendWriteWithoutResponse peripheral: CBPeripheral) {
        self.delegates.do{ $0.onIsReadyToSendWriteWithoutResponse?(peripheral: peripheral) }
    }
}

// MARK: send data & receive data
extension BLECenter {
    
    func iSendData(data: Any,
                   service: String,
        ...
        var cmdData:Data?
        if isData {
            cmdData = data as? Data
        } else {
            let iBytes:[UInt8] = data as? [UInt8] ?? []
            if iBytes.count > 0 {
                cmdData = Data(bytes: iBytes, count: iBytes.count)
            }
        }
        
        guard let cmdData = cmdData else {
        ...
        if value.sussess {
            self.connectedDevice?.blePeripheral?.writeValue(cmdData, for: value.cha!, type: changeReadWriteType(type: type))
    ...
    func peripheral(_ peripheral: CBPeripheral, didWriteValueFor characteristic: CBCharacteristic, error: Error?) {
        ...
        self.delegates.do { $0.onWriteCharacteristicValue?(cha: characteristic, error: error)}
    }
    ...
    func peripheral(_ peripheral: CBPeripheral, didUpdateValueFor characteristic: CBCharacteristic, error: Error?) {
        ...
        self.delegates.do { $0.onUpdateCharacteristicValue?(cha: characteristic, error: error)}
...
// MARK: format uuid & response type
extension BLECenter {
...
extension BLECenter {
    ...
    func iOta(filePath:String, progressCallback:BLECALLBACK?, callback: BLECALLBACK?) {
        ...
        otaMgr!.otaStart(filePath, progressCallback, {response in
            self.otaMgr = nil
            callback?(response)
        })
    }
    
    func iOtaWrite(data :Data, characteristic:CBCharacteristic, type: CBCharacteristicWriteType) {
        characteristic.service?.peripheral?.writeValue(data, for: characteristic, type: type)
    }
    
    func iOtaRead(characteristic:CBCharacteristic) {
...
extension BLECenter {
    ...
    func connectWithServiceUUIDs(services: [String],
                                 scanService:String? = nil,
                                 deviceName:String?=nil,
                                 timeout: Int = 20,
                                 callback:TaskCallback? = nil) {
        var parameters = [BLEConstants.SERVICE_UUIDS : services, BLEConstants.BLE_TIMEOUT: timeout] as [String : Any]
        if let scanService = scanService {
            parameters[BLEConstants.BLE_SCAN_SERVICE] = scanService
        }
        
        if let deviceName = deviceName {
            parameters[BLEConstants.BLE_DEVICE_NAME] = deviceName
        }
    ...
    func gotoOta(filePath:String, progressCallback:BLECALLBACK? = nil, callback:BLECALLBACK? = nil) {
        self.tasksCenterMgr.executeSystemSyncTask(type: .ota,
                                                  priority: .emergency,
